...
if(~exist('log','var'))
    if(ego_vs_ego)
        [opp_file,path] = uigetfile(fullfile(get_bags_path,'*.mat'),'Load ego2 log');
    else
        [opp_file,path] = uigetfile(fullfile(get_bags_path,'*.mat'),'Load opponent log');
    end
...
if(multi_run)
    if(~exist('log_ego','var'))
        [file,path] = uigetfile(fullfile(get_bags_path,'*.mat'),'Load ego log');
